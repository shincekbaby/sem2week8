...
{	
	...
	int linedisp(Node *curr, int k, int l, int &c);
};
...
int Btree::range_search(int k, int l)
{
	...
	cout<<" DONE ,, No of value in range = "<<c;
	
	return 0;
}

int Btree::linedisp(Node *curr, int k, int l, int &c)
{
	if( curr->left && curr->left->data >=k)
	linedisp(curr->left,k,l,c);
	
	cout<<curr->data<<",";++c;
	
	if( curr->right && curr->right->data <=l)
	linedisp(curr->right,k,l,c);
